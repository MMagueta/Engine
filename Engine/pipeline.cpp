...
namespace engine {
    ...
    void Pipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfiguration& config){
        ...
        assert(config.renderPass != VK_NULL_HANDLE && "Cannot create graphics pipeline since no renderpass was provided.");
    ...
    PipelineConfiguration Pipeline::defaultPipelineConfiguration(uint32_t width, uint32_t height){
        PipelineConfiguration configInfo{};

        // First stage of the pipeline
        // Picks the list of numbers and feeds to a triangle geometry
